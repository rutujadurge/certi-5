...
import pdfplumber
def generate_certificate(output_path, certificate_id, uid, candidate_name, course_name, org_name, institute_logo_path):
    ...
    # Creating a clickable link for certificate_id
    certificate_link = f"<a href='http://localhost:8501/verifier?certificate_id={certificate_id}'>{certificate_id}</a>"
    recipient_text = f"This is to certify that<br/><br/>\
                     <font color='red'> {candidate_name} </font><br/>\
                     with UID <br/> \
                    <font color='red'> {uid} </font> <br/><br/>\
                     has successfully completed the course:<br/>\
                     <font color='blue'> {course_name} </font><br/><br/>\
                     Certificate ID:<br/> \
                    <font color='orange'> {certificate_link}</font>"
